...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 1. 刷新前准备, 设置close、active标记
			// 提供了自定义属性处理 和 属性验证功能
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 2. 创建bean工厂，并解析配置文件为beandefinition，之后将bean定义注册到bean工厂
			...
			try {
				...
				// 调用BeanDefinitionRegistryPostProcessor和普通的BeanFactoryPostProcessor
				// springboot是在这里进行beandefainition扫描的
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		if (!shouldIgnoreSpel) {
			// 表达式语言处理器，supporting "#{...}" expressions in a Unified EL compatible style
			beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		}
		// 属性编辑器，主要对bean的属性等设置进行管理的一个工具
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		// 加入ApplicationContextAwareProcessor，用于aware接口方法回调
		// 正是由于加入了这个Processor,通过这个Processor进行调用的接口需要忽略注入
		...
		// 注册一个ApplicationListener接口检测的BeanPostProcessor，如果是ApplicationListener，加入订阅者列表
